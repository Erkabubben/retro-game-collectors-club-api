...
router.get('/:id', auth.authenticateJWT, (req, res, next) => controller.findWebhook(req, res, next))

router.post('/', auth.authenticateJWT, (req, res, next) => controller.create(req, res, next))

router.delete('/:id', auth.authenticateJWT, auth.ensureUserIsResourceOwner, (req, res, next) => controller.delete(req, res, next))
